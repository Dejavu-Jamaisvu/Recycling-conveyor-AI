...
# =====================================================================
# 이 스크립트는 Colab이 아니라 실제 Jetson Nano 위에서 실행합니다.
#
# 사전 준비물 (같은 폴더에 넣어두기)
#   - best.pt            (1번 스크립트 결과물: YOLO 위치 검출 모델)
...
YOLO_CONF_THRESHOLD = 0.4             # YOLO 검출 최소 신뢰도
...
# YOLO 위치 검출 + 크롭
# ---------------------------------------------------------------------
def detect_and_crop(yolo_model, frame):
...
# CNN 분류
# ---------------------------------------------------------------------
def classify(interpreter, crop, class_names):
    ...
    try:
        while True:
            ...
            if GPIO.input(IR_SENSOR_PIN) == GPIO.HIGH:
                ...
                # 3) YOLO로 위치 검출 + 크롭
                crop, yolo_conf = detect_and_crop(yolo_model, frame)
                ...
                # 4) CNN으로 최종 분류
                cnn_class, cnn_conf = classify(cnn_interpreter, crop, class_names)
                ...
                conn.execute(
                    ...
                    INSERT INTO sorting_log
                        (timestamp, yolo_confidence, cnn_class, cnn_confidence, final_class, servo_used)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (
