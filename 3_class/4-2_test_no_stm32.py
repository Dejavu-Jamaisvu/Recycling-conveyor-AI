...
#
# 실제 STM32의 "TRIGGER" 신호 대신, 터미널에서 Enter 키를 누르면 그 순간
# 촬영 -> 분류를 수행합니다. 시리얼(pyserial)도, GPIO도 전혀 사용하지 않습니다.
#
...
def main():
    ...
    print("\n준비 완료. Enter 키 = 촬영+분류 (STM32의 TRIGGER 대신), q + Enter = 종료\n")
    try:
        while True:
            cmd = input("촬영하려면 Enter (종료: q) > ").strip().lower()
            if cmd == "q":
                break
...
# =====================================================================
# 사용 방법
#   1) python3 4-2_test_no_stm32.py 실행
#   2) 물체를 카메라 앞에 놓고 Enter -> 분류 결과 확인
